...
internal sealed class FakeWfp : IWfpOps
{
    ...
    public IReadOnlyDictionary<FilterGroup, int> InstalledCounts() => Groups.ToDictionary(g => g.Key, g => g.Value.Count);
...
internal sealed class FakeRas(FakeInventory inventory) : IRasOps
{
    ...
    public Task<RasDialResult> DialAsync(string entryName, string userName, ReadOnlyMemory<char> password, string? domain, CancellationToken cancellationToken)
    {
        ...
        return Task.FromResult(result with { Handle = result.Success ? new RasConnectionHandle(HandleBase + slot) : null });
    }
...
internal sealed class FakeProxy : IDnsProxy
{
    ...
    public void Pin(string name, IReadOnlyList<uint> addresses) => Pinned[name] = addresses;
...
internal sealed class FakeProbes : IProbeOps
{
    ...
    public Task<ServerCertificateFacts?> ServerCertificateAsync(uint remote, ushort port, string? host, TimeSpan timeout, CancellationToken cancellationToken)
    {
        ...
        return Task.FromResult(ServerCertificate);
    }
