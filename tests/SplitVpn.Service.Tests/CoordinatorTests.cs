...
public sealed partial class CoordinatorTests : IDisposable
{
    ...
    /// <summary>
    /// Фильтры Direct считаются перечислением всех фильтров системы (около 300 мс), поэтому их пропажа
    /// замечается не каждой полной сверкой, а раз в DirectFilterCheckInterval.
    /// </summary>
    [Fact]
    public async Task PeriodicReconcile_ChecksDirectFilters_OncePerInterval()
    {
        var coordinator = await StartAsync();
        await ConnectAsync(coordinator);
        ...
        var counts = _world.Wfp.DirectCounts;
        ...
        Assert.Equal(counts, _world.Wfp.DirectCounts);
        Assert.Equal(replacements, _world.Wfp.Replacements);
        Assert.Equal(directCount - 1, _world.Wfp.Groups[FilterGroup.Direct].Count);

        _world.Time.Advance(Coordinator.DirectFilterCheckInterval);
        await TickAsync(coordinator);

        Assert.Equal(counts + 1, _world.Wfp.DirectCounts);
    ...
    public async Task PeriodicReconcile_RestoresExternalDrift()
    {
        var coordinator = await StartAsync();
        await ConnectAsync(coordinator);
        var baseCount = _world.Wfp.Groups[FilterGroup.Base].Count;
        var routeCount = _world.Routes.Current.Count;
        var replacements = _world.Wfp.Replacements;

        _world.Wfp.RemoveOne(FilterGroup.Base);
        ...
        Assert.Equal(baseCount, _world.Wfp.Groups[FilterGroup.Base].Count);
    ...
    [Fact]
    public async Task PrivateNetworkRule_RoutesRemoteLanIntoItsTunnel_AndWarnsAboutAdapterNetwork()
    {
        var second = SecondProfile();
        SaveSettings(Settings() with
        {
            ...
            Rules =
            [
                new UserRuleSetting { Cidr = "192.168.100.0/24", Target = RouteTarget.Tunnel(second.Id) },
                new UserRuleSetting { Cidr = "192.168.1.0/25", Target = RouteTarget.Tunnel(second.Id) },
            ],
        });
        var coordinator = await StartAsync();
        ...
        var routes = _world.Routes.On(FakeInventory.TunnelLuid(1));
        Assert.Contains(Ipv4Cidr.Parse("192.168.100.0/24"), routes);
        Assert.DoesNotContain(routes, r => r.ToRange().Overlaps(Ipv4Cidr.Parse("192.168.1.0/24").ToRange()));
        var warning = Assert.Single(coordinator.BuildStatus().Warnings, w => w.Code == "rule-onlink");
        Assert.Contains("192.168.1.0/25", warning.Text, StringComparison.Ordinal);
    }
    ...
    private static async Task CompleteDialAsync(Coordinator coordinator)
    {
        ...
        for (var i = 0; i < 600 && quiet < 2; i++)
        {
            ...
            if (coordinator.Facts.Tunnels.Values.Any(t => t.Dialing || t.Verifying || t.CertificateProbeRunning || t.ServerResolving))
            {
                quiet = 0;
                await Task.Delay(5);
                continue;
