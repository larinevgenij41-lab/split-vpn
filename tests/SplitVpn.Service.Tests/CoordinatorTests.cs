...
public sealed partial class CoordinatorTests : IDisposable
{
    ...
    public async Task DomainRule_PinsRoute_AndDropsItWhenTtlExpires()
    {
        ...
        await coordinator.ObservePinnedAsync(new PinnedRouteNotice("www.example.org", RouteTarget.Direct, [address], TimeSpan.FromMinutes(5)), CancellationToken.None);
    ...
    /// <summary>
    /// Дозвон идёт в фоновой задаче и возвращает результат в очередь актора. Обработка результата
    /// может начать дозвон следующего туннеля, поэтому ждём, пока и очередь, и дозвоны не затихнут
    /// дважды подряд.
    /// </summary>
    private static async Task CompleteDialAsync(Coordinator coordinator)
    {
        ...
        for (var i = 0; i < 600 && quiet < 2; i++)
        {
            ...
            if (coordinator.Facts.Tunnels.Values.Any(t => t.Dialing || t.CertificateProbeRunning))
            {
                quiet = 0;
                await Task.Delay(5);
                continue;
