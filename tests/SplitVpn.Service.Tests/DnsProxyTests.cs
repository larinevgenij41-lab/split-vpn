...
public sealed class DnsProxyTests : IAsyncDisposable
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task StaleDomainAnswer_RequiresConfirmedRoute(bool applied)
    {
        var sink = new RecordingSink { Applied = applied };
        _proxy.RouteSink = sink;
        _proxy.Configuration = new DnsProxyConfiguration
        {
        ...
        var query = DnsMessage.BuildQuery(1, CorporateName, DnsMessage.TypeA);
        DnsMessage.TryReadQuestion(query, out var question);
        _cache.Put(question, _upstream.Context, DnsMessage.BuildAddressResponse(query, [Ipv4.Parse("203.0.113.9")], 60));
        ...
        Assert.Equal(applied ? 0 : 2, DnsMessage.GetRcode(response));
    }
    ...
    [InlineData(300u)]
    [InlineData(0u)]
    public async Task DomainAnswer_TtlDoesNotOutliveRouteOrOriginalAnswer(uint ttl)
    {
        var sink = new RecordingSink();
        _proxy.RouteSink = sink;
        _proxy.Configuration = new DnsProxyConfiguration
        ...
        Assert.NotNull(sink.Last);
        Assert.True(DnsMessage.GetMinTtl(response) <= Math.Min(ttl, sink.Last.Ttl.TotalSeconds));
    }

    [Theory]
    [InlineData(2, 0, 1)]
    [InlineData(3, 3, 0)]
    public async Task Forwarder_RetriesServfailButKeepsNxDomain(int firstCode, int expectedCode, int secondQueries)
    {
        await using var second = new FakeUpstream();
        _upstream.Rcode = firstCode;
        var query = DnsMessage.BuildQuery(1, CorporateName, DnsMessage.TypeA);

        var response = await DnsForwarder.ForwardAsync(query, [_upstream.EndPoint, second.EndPoint], null,
            TimeSpan.FromSeconds(1), CancellationToken.None);

        Assert.NotNull(response);
        Assert.Equal(expectedCode, DnsMessage.GetRcode(response));
        Assert.Equal(secondQueries, second.Received);
    }
    ...
    private sealed class FakeUpstream : IAsyncDisposable
    {
        ...
        private async Task LoopAsync(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                var answered = Rcode != 0 ? DnsMessage.BuildEmptyResponse(query, Rcode) : AnswerAs is { } name
