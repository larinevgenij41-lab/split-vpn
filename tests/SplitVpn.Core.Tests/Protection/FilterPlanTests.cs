...
public partial class FilterPlanTests
{
    ...
    public void Groups_AreSeparatedByLifetime()
    {
        ...
        Assert.All(FilterPlanBuilder.BuildDirect(inputs), f => Assert.Equal(
            f.Action == FilterAction.Permit ? FilterPlanBuilder.WeightDirect : FilterPlanBuilder.WeightRouteGuard, f.Weight));
    ...
    private static bool Matches(FilterCondition condition, Packet packet) => condition switch
    {
        ...
        RemotePortCondition p => p.NotEqual ? packet.RemotePort != p.Port : packet.RemotePort == p.Port,
