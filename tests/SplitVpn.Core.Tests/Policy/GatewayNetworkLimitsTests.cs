...
public sealed class GatewayNetworkLimitsTests
{
    ...
    /// <summary>
    /// Правило пользователя действует и в частных сетях, поэтому его запрет там сохраняется поверх сети шлюза
    /// так же, как в публичных адресах; остальная частная сеть шлюза уходит в его туннель.
    /// </summary>
    [Fact]
    public void Compiler_KeepsUserBlockInsidePrivateRanges()
    {
        ...
        Assert.Equal(Decision.Block, policy.Classify(Ipv4.Parse("10.5.1.1")).Decision);
        Assert.Equal(Gateway, policy.Classify(Ipv4.Parse("10.6.1.1")).Tunnel);
    }
}
