...
public class PolicyCompilerTests
{
    ...
    [Fact]
    public void ServerAndLocal_OverrideUserBlock()
    {
        var policy = PolicyCompiler.Compile(new PolicyInput
        {
            DefaultTarget = ToA,
        ...
        Assert.Equal(Decision.Local, policy.Classify(Ipv4.Parse("192.168.1.1")).Decision);
    ...
    private static Classification? NaiveSpecial(PolicyInput input, uint address)
    {
        ...
        if (SpecialRanges.Local.Contains(address))
        {
    ...
    {
        var zones = new[] { 0x2D000000u, 0x2E000000u, 0xC0A80000u, 0xCB007100u };
