...
var commands = new Dictionary<string, Func<CliArgs, Task<int>>>(StringComparer.OrdinalIgnoreCase)
{
    ...
    ["tunnel-pause"] = ServiceCommands.TunnelPauseAsync,
    ["tunnel-resume"] = ServiceCommands.TunnelResumeAsync,
    ["profile-order"] = ServiceCommands.ProfileOrderAsync,
...
if (args.Length == 0 || !commands.TryGetValue(args[0], out var command))
{
    ...
    Console.WriteLine("  tunnels | tunnel-pause --profile <id> | tunnel-resume --profile <id>");
    Console.WriteLine("  profile-order --ids <id1,id2,...>     порядок подключений и очерёдность подъёма");
