...
public sealed partial class Coordinator
{
    ...
    internal async Task TickAsync(CancellationToken cancellationToken)
    {
        ...
        if (changed || periodic || (!paused && (Facts.PartialError is not null || DialDue(now))))
        {
...
internal sealed class CoordinatorFacts
{
    ...
    /// <summary>
    /// Адреса из правил для доменов: читаются вне очереди актора (быстрая проверка в DNS-посреднике),
    /// меняются только в очереди.
    /// </summary>
    public ConcurrentDictionary<uint, PinnedRoute> Pinned { get; } = new();
