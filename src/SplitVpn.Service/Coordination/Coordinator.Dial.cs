...
public sealed partial class Coordinator
{
    ...
    private async Task EnsureDialsAsync(CancellationToken cancellationToken)
    {
        ...
        {
            if (tunnel.Protocol == VpnProtocol.AnyConnect && !tunnel.IsUp)
            {
            ...
            if (tunnel.IsUp)
            {
                if (!tunnel.Verified)
                {
                    await VerifyAsync(tunnel, cancellationToken);
                }
    ...
    private bool CanStartDial(TunnelFacts tunnel, out ConnectionProfile profile, out char[] password)
    {
        profile = null!;
        password = [];
        if (tunnel.Dialing || Facts.ProfileTestRunning || tunnel.BlockingError != ErrorCategory.None || tunnel.ExternallyDisconnected
    ...
    internal async Task OnDialFinishedAsync(Guid profileId, RasDialResult result, int generation, CancellationToken cancellationToken)
    {
        ...
        if (generation != tunnel.DialGeneration || _state.Intent != Intent.Connected)
        {
    ...
    /// </summary>
    private TimeSpan ScheduleRetry(TunnelFacts tunnel, DateTimeOffset now)
    {
    ...
    /// </summary>
    private async Task VerifyAsync(TunnelFacts tunnel, CancellationToken cancellationToken)
    {
        ...
        var foreign = await ProbeAsync(_settings.CheckTargets.Foreign, adapter.Addresses, cancellationToken);
        var russianOk = !RussianCheckNeeded()
            || await ProbeAsync(_settings.CheckTargets.Russian, Facts.Primary.Addresses, cancellationToken);
        if (foreign)
        {
    ...
    private async Task RegisterVerificationFailureAsync(TunnelFacts tunnel, CancellationToken cancellationToken)
    {
        ...
        tunnel.LastErrorCode = null;
        var delay = ScheduleRetry(tunnel, now);
