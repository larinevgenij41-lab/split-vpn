...
public sealed partial class Coordinator
{
    ...
    private void EnsureProtection()
    {
        ...
        var changed = new Dictionary<FilterGroup, IReadOnlyList<FilterSpec>>
        {
            [FilterGroup.Base] = FilterPlanBuilder.BuildBase(inputs),
            [FilterGroup.Runtime] = FilterPlanBuilder.BuildRuntime(inputs),
            [FilterGroup.Dynamic] = FilterPlanBuilder.BuildDynamic(inputs),
            [FilterGroup.Direct] = FilterPlanBuilder.BuildDirect(inputs),
        };
        var fingerprints = new Dictionary<FilterGroup, string>();
        foreach (var (group, specs) in changed.ToList())
        {
            if (ChangedGroupFingerprint(group, specs, installed) is { } fingerprint)
            {
        ...
        foreach (var (group, fingerprint) in fingerprints)
        {
            Facts.AppliedFilters[group] = (fingerprint, counts[group]);
        }
    }
    ...
    /// </summary>
    private string? ChangedGroupFingerprint(FilterGroup group, IReadOnlyList<FilterSpec> specs, IReadOnlyDictionary<FilterGroup, int>? installed)
    {
