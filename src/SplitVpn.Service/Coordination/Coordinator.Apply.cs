...
public sealed partial class Coordinator
{
    ...
    private async Task ApplyAsync(CancellationToken cancellationToken)
    {
        ...
        if (await EnsureServerAddressesAsync(cancellationToken))
        {
            timing.Mark("адреса серверов");
            CompilePolicy();
            EnsureProtection();
            EnsureRoutes();
            timing.Mark("повтор политики, WFP и маршрутов");
        }
    ...
    private void CompilePolicy()
    {
        ...
        Facts.Policy = PolicyCompiler.Compile(new PolicyInput
        {
    ...
    private static IEnumerable<uint> ServerAddressesOf(TunnelFacts tunnel) =>
        tunnel.AnyConnect is { ResolvedAddresses.Count: > 0 } link ? tunnel.ServerAddresses.Union(link.ResolvedAddresses) : tunnel.ServerAddresses;
    ...
    private void EnsureProtection()
    {
        ...
        var installed = Facts.ForceProtection ? _deps.Wfp.InstalledCounts() : null;
        ApplyGroup(FilterGroup.Base, FilterPlanBuilder.BuildBase(inputs), installed);
        ApplyGroup(FilterGroup.Runtime, FilterPlanBuilder.BuildRuntime(inputs), installed);
        ApplyGroup(FilterGroup.Dynamic, FilterPlanBuilder.BuildDynamic(inputs), installed);
        ApplyGroup(FilterGroup.Direct, FilterPlanBuilder.BuildDirect(inputs), installed);
    }

    /// <summary>Заменяет группу, если изменился план или (при полной сверке) число фильтров в системе.</summary>
    private void ApplyGroup(FilterGroup group, IReadOnlyList<FilterSpec> specs, IReadOnlyDictionary<FilterGroup, int>? installed)
    {
        var fingerprint = FilterFingerprint(specs);
        var unchanged = Facts.AppliedFilters.TryGetValue(group, out var applied) && applied.Fingerprint == fingerprint;
        var drifted = unchanged && installed is not null && installed.GetValueOrDefault(group) != applied.Count;
        ...
        var count = _deps.Wfp.ReplaceGroup(group, specs);
        Facts.AppliedFilters[group] = (fingerprint, count);
    }
    ...
    private void EnsureRoutes()
    {
        ...
        if (Facts.Primary is { DefaultGateway: { } gateway } primary)
        {
            ...
            {
                desired.AddRange(Facts.Policy!.DirectRouteCidrs.Select(c => new RouteKey(c, gateway, primary.Luid)));
            }
        ...
        foreach (var tunnel in Facts.Tunnels.Values.Where(t => t.Luid is not null))
        {
            ...
            {
                desired.AddRange(Facts.Policy!.RouteCidrsFor(tunnel.ProfileId).Select(c => new RouteKey(c, 0, luid)));
            }
    ...
    /// <summary>Адреса из правил для доменов, закреплённые за указанной целью.</summary>
    private List<Ipv4Cidr> PinnedCidrs(RouteTarget target)
    {
        // Адрес VPN-сервера не уводится доменным правилом в туннель: суффикс вроде «example.org» накрыл бы и шлюз.
        var servers = AllServerAddresses();
        return Facts.Pinned.Where(p => p.Value.Target == target && !servers.Contains(p.Key)).Select(p => new Ipv4Cidr(p.Key, 32)).ToList();
    }
    ...
    /// <summary>Адреса серверов всех туннелей: IP из профиля, кеш или начальное разрешение. true — что-то изменилось.</summary>
    private async Task<bool> EnsureServerAddressesAsync(CancellationToken cancellationToken)
    {
        var changed = false;
        foreach (var tunnel in Facts.Tunnels.Values.ToList())
        {
            changed |= await EnsureServerAddressesAsync(tunnel, cancellationToken);
        }
    ...
    private async Task<bool> EnsureServerAddressesAsync(TunnelFacts tunnel, CancellationToken cancellationToken)
    {
        ...
        else if (!ResolveDue(tunnel))
        {
            return false;
        }
        else
        {
            addresses = await ResolveServerAsync(tunnel, server.Host, cancellationToken);
        ...
        tunnel.ServerAddresses = addresses;
    ...
    private void RememberFingerprint(TunnelFacts tunnel, string fingerprint)
    {
        ...
        others.Add(current with { Fingerprint = fingerprint });
    ...
    {
        var stale = _deps.Time.GetUtcNow() - tunnel.ServerResolvedUtc > ServerResolveInterval;
        return tunnel.ServerAddresses.Count == 0 || stale || tunnel.FailedDialsSinceResolve >= 3;
    }

    private async Task<IReadOnlyList<uint>> ResolveServerAsync(TunnelFacts tunnel, string host, CancellationToken cancellationToken)
    {
        ...
        var addresses = await _deps.Resolver.ResolveAsync(host, ServiceDnsAddresses(), Facts.Primary.InterfaceIndex, cancellationToken);
    ...
    private List<DomainRoute> BuildDomainRoutes()
    {
        ...
        foreach (var rule in _settings.DomainRules)
        {
            ...
            routes.Add(new DomainRoute(suffix, rule.Target, DomainDnsRoute(rule.Target)));
        }
    ...
    /// и получает разрешающий фильтр. Правило пользователя на то же имя важнее: оно уже в списке.
    /// </summary>
    private void AddRevocationDomainRoutes(List<DomainRoute> routes)
    {
        foreach (var host in Facts.Tunnels.Values.SelectMany(t => t.RevocationHosts).Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            if (suffix.Length == 0 || routes.Exists(r => r.Suffix == suffix))
            {
                continue;
            }

            routes.Add(new DomainRoute(suffix, RouteTarget.Direct, DomainDnsRoute(RouteTarget.Direct)));
        }
    }
    ...
    private static string DescribeDomains(DnsProxyConfiguration configuration) =>
        string.Join("|", configuration.DomainRoutes.Select(d => d.Suffix + ">" + d.Target + ">" + d.PinAddresses + ">" + Describe(d.Route)));
    ...
    {
        if (Facts.Policy is not null && _state.Intent != Intent.Off)
        {
            var timing = new ApplyTiming();
            CompilePolicy();
            timing.Mark("политика");
    ...
    private static string FilterFingerprint(IReadOnlyList<FilterSpec> specs) =>
