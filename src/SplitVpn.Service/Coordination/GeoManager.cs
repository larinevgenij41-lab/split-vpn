...
internal sealed class GeoManager(ServiceDependencies deps, GeoListKind kind = GeoListKind.Geo)
{
    ...
    public Task TickAsync(Coordinator coordinator, CancellationToken cancellationToken)
    {
        ...
        if (!settings.AutoUpdate || !due || _fetch is { IsCompleted: false } || !PathAllowed(coordinator))
        {
    ...
    private static bool PathAllowed(Coordinator coordinator) =>
        coordinator.State.Intent == Intent.Off || coordinator.State.ProtectionSuspended
        || (coordinator.Facts.Tunnels.Count > 0 && coordinator.Facts.Tunnels.Values.All(t => t.Verified));
    ...
    private async Task FetchAsync(Coordinator coordinator, CancellationToken cancellationToken)
    {
        ...
        {
            fetch = new GeoFetchResult { Status = GeoFetchStatus.Failed, Failure = GeoFetchFailure.Network, Message = ex.Message };
        }
