...
public sealed record AppSettings
{
    ...
    /// <summary>Все подключения, которые служба поднимает: опорное первым.</summary>
    [JsonIgnore]
    public IReadOnlyList<ConnectionProfile> ActiveProfiles =>
        Profiles.Where(p => p.Role != ProfileRole.Off).OrderByDescending(p => p.Role == ProfileRole.Primary).ToList();
