...
public static class DnsMessage
{
    ...
    /// <summary>A-записи ответа (для начального разрешения имени сервера).</summary>
    public static IReadOnlyList<uint> ReadAddresses(ReadOnlySpan<byte> message)
    {
        var result = new List<uint>();
        var copy = message.ToArray();
        VisitRecords(copy, includeAdditional: false, (type, ttlOffset, bytes) =>
        {
            var length = BinaryPrimitives.ReadUInt16BigEndian(bytes[(ttlOffset + 4)..]);
            if (type == TypeA && length == 4)
            {
                result.Add(BinaryPrimitives.ReadUInt32BigEndian(bytes[(ttlOffset + 6)..]));
            }
        });
        return result;
    }
