...
public static class IpcNames
{
    ...
    // 7: выключатель отдельного туннеля и порядок подключений (SetTunnelPaused, SetProfileOrder).
...
[JsonDerivedType(typeof(SetTunnelPausedRequest), "setTunnelPaused")]
[JsonDerivedType(typeof(SetProfileOrderRequest), "setProfileOrder")]
...
/// <summary>
/// Положить или поднять один туннель прямо сейчас, не трогая настройки. В отличие от смены роли
/// на «Выключено», профиль остаётся включённым и назначенные ему цели не осиротеют.
/// </summary>
public sealed record SetTunnelPausedRequest(Guid ProfileId, bool Paused) : IpcRequest;

/// <summary>Переставить подключения: порядок списка задаёт и вид на экране, и очерёдность подъёма.</summary>
public sealed record SetProfileOrderRequest(IReadOnlyList<Guid> Order) : IpcRequest;
...
public sealed record TunnelStatusDto
{
    ...
    /// <summary>Туннель положен пользователем и не дозванивается. Настройки при этом не менялись.</summary>
