...
public sealed partial class IpcClient : IAsyncDisposable
{
    ...
    /// <summary>
    /// Запрос с пределом ожидания. После истечения предела соединение больше не годится: ответ на
    /// брошенный запрос придёт позже и перепутает кадры, поэтому вызывающий обязан закрыть клиента.
    /// </summary>
