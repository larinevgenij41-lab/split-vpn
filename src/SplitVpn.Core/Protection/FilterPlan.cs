...
/// <summary>Адрес, полученный по правилу для домена, и его цель.</summary>
public sealed record PinnedHost(uint Address, RouteTarget Target);
...
public sealed record ProtectionInputs
{
    ...
    /// <summary>Адреса из правил для доменов, закреплённые за целью на время TTL.</summary>
    public IReadOnlyList<PinnedHost> PinnedHosts { get; init; } = [];
...
public static class FilterPlanBuilder
{
    ...
    /// <summary>Разрешения и блокировки по адресам, полученным из правил для доменов.</summary>
    public static IReadOnlyList<FilterSpec> BuildDynamic(ProtectionInputs inputs)
    {
        ArgumentNullException.ThrowIfNull(inputs);
        var filters = new List<FilterSpec>();
        foreach (var host in inputs.PinnedHosts.DistinctBy(h => (h.Address, h.Target)))
        {
            var name = Ipv4.Format(host.Address);
            if (host.Target.Kind == TargetKind.Direct)
            {
                filters.Add(Spec("Домен напрямую " + name, FilterGroup.Dynamic, FilterFamily.V4, WeightDirect, FilterAction.Permit,
                    new RemoteRangeV4(host.Address, host.Address)));
            }
            else if (host.Target.Kind == TargetKind.Block)
            {
                filters.Add(Spec("Домен заблокирован " + name, FilterGroup.Dynamic, FilterFamily.V4, WeightUserBlock, FilterAction.Block,
                    new RemoteRangeV4(host.Address, host.Address)));
            }
        }
