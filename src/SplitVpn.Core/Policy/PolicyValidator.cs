...
public static class PolicyValidator
{
    ...
    {
        var protectedSet = SpecialRanges.Local.Union(SpecialRanges.Loopback).Union(RangeSet.From(serviceAddresses.Select(Ipv4Range.Host)));
        foreach (var rule in rules.Where(r => r.Cidr.PrefixLength > 0 && protectedSet.Overlaps(r.Cidr.ToRange())))
        {
            warnings.Add(Text($"Правило {rule.Cidr} пересекается с локальными или служебными адресами; для них правило не действует."));
        }
    }
