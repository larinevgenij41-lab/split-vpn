...
internal sealed class IntervalPainter
{
    ...
    public void Paint(RangeSet ranges, Classification label)
    {
        ...
        // Один диапазон (loopback, сеть адаптера, адрес сервера) меняет только затронутый участок.
        // Полная копия десятков тысяч сегментов здесь дороже точечной вставки.
        if (ranges.Count == 1)
        {
            Paint(ranges[0], label);
            return;
        }
