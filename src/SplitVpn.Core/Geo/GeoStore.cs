...
public sealed record GeoStoreState
{
    ...
    /// <summary>Адрес, выдавший ETag: валидатор другого URL нельзя отправлять новому источнику.</summary>
    public string? LastETagUrl { get; init; }
}
...
public sealed class GeoStore
{
    ...
    /// <summary>Ошибка уборки старых ревизий не отменяет уже записанное состояние.</summary>
    ...
    // Ранее скачанная версия могла быть пропущена: после разрешения нужен её текст, а не HTTP 304.
    public GeoStoreState ClearSkipped() => Commit(LoadState() with { Skipped = [], LastETag = null, LastETagUrl = null });
    ...
    private void RemoveUnreferenced(GeoStoreState state)
    {
        ...
        foreach (var dir in Directory.GetDirectories(RevisionsRoot))
        {
            if (!keep.Contains(Path.GetFileName(dir)))
            {
                ...
                {
                    // Следующая фиксация состояния повторит уборку; остальные каталоги удаляем сейчас.
                    OnCleanupError?.Invoke(ex);
                }
            }
        }
