...
public sealed record TunnelRow(string Name, ProfileRole Role, string RoleText, string Protocol, string State, string Detail, string? Note, StateTone Tone)
{
    /// <summary>Имя строки для экранного диктора.</summary>
    ...
    public static TunnelRow From(TunnelStatusDto tunnel, string? speed, bool nothingRouted)
    {
        ...
        return new TunnelRow(
            ...
            StatusText.StateName(tunnel.State),
            string.Join(" · ", parts),
            nothingRouted ? "на это подключение ничего не направлено" : null,
            tone);
    }
