...
public sealed class DnsProxyServer : IAsyncDisposable
{
    ...
    public async Task<byte[]?> HandleAsync(byte[] query, CancellationToken cancellationToken)
    {
        ...
        var choice = SelectRoute(configuration, question.Name);
        ...
        if (response is null)
        {
            ...
            return _cache.TryGet(question, context, id, offline: true)
                ?? (choice.Route is null ? _cache.TryGetAnyPath(question, id) : null)
                ?? DnsMessage.BuildServerFailure(query);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (ReferenceEquals(Configuration, configuration))
        {
            try
            {
                if (await RouteSink.ObserveAsync(new PinnedRouteNotice(question.Name, domain.Suffix, domain.Target, addresses, RouteTtl(response)), cancellationToken))
                {
                    return response;
                }
            }
    ...
    private async Task UdpLoopAsync(Socket socket, CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            _ = Task.Run(() => ReplyUdpAsync(socket, query, received.RemoteEndPoint, token), CancellationToken.None);
        }
    }
    ...
    private async Task TcpAcceptLoopAsync(TcpListener listener, CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            _ = Task.Run(() => ServeTcpAsync(client, token), CancellationToken.None);
        }
    }
