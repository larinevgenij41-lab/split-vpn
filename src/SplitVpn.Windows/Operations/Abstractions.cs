...
{
    /// <summary>Атомарно заменяет переданные группы; возвращает число объектов WFP в каждой изменённой группе.</summary>
    IReadOnlyDictionary<FilterGroup, int> ReplaceGroups(IReadOnlyDictionary<FilterGroup, IReadOnlyList<FilterSpec>> groups);

    /// <summary>
    /// Число установленных объектов WFP по группам: расхождение означает изменение извне. Без includeDirect
    /// группа Direct в ответ не входит: её десятки тысяч фильтров считаются только перечислением всех фильтров
    /// системы, а это около 300 мс. Значение -1 означает изменение набора ID при полной сверке:
    /// группа требует восстановления даже при прежнем количестве объектов.
    /// </summary>
    IReadOnlyDictionary<FilterGroup, int> InstalledCounts(bool includeDirect);
...
public interface IDnsProxy
{
    ...
    void Pin(string name, IReadOnlyList<uint> addresses, TimeSpan? lifetime = null);

    void RetainPins(IEnumerable<string> permanentNames);
