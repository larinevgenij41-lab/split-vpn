...
public sealed class SystemWfpOps(WfpIdentity identity) : IWfpOps
{
    ...
    public IReadOnlyDictionary<FilterGroup, int> ReplaceGroups(IReadOnlyDictionary<FilterGroup, IReadOnlyList<FilterSpec>> groups)
    {
        ...
        var installed = WfpOps.ListFilters(engine, identity);
        ...
        engine.InTransaction(e =>
        {
            foreach (var (group, specs) in groups)
            {
                ...
                {
                    counts[group] = ReplaceByDifference(e, own, named);
                    continue;
                }

                WfpOps.DeleteFilters(e, own.Select(f => f.Id));
                var added = WfpOps.AddFilters(e, identity, named, persistent: group == FilterGroup.Base, indexed: false);
    ...
    private int ReplaceByDifference(WfpEngine engine, List<WfpFilterInfo> installed, List<FilterSpec> named)
    {
        ...
        WfpOps.DeleteFilters(engine, difference.Stale);
        var added = WfpOps.AddFilters(engine, identity, difference.Missing, persistent: false, indexed: true).Count;
    ...
    public IReadOnlyDictionary<FilterGroup, int> InstalledCounts(bool includeDirect)
    {
        ...
        var engine = Engine();
        return small.ToDictionary(g => g, g => _ids[g].Count(id => WfpOps.FilterExists(engine, id)));
    }
    ...
    {
        WfpOps.DeleteAll(Engine(), identity);
        _ids.Clear();
    }
